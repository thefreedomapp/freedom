const quotes = [
  ...
  { quote: 'The basis of a democratic state is liberty.', author: 'Aristotle' },
  {
    quote:
  ...
  { quote: 'Only the educated are free.', author: 'Franklin D. Roosevelt' },
  {
    quote:
  ...
  {
    quote: 'This liberty will look easy by and by when nobody dies to get it.',
  ...
  app.get('/api/quote', (req, res) =>
    req.query.amount
      ? (() => {
          ...
          for (let i = 0; i < parseInt(req.query.amount); i++)
            _quotes.push(quotes[~~(quotes.length * Math.random())]);

          res.json(_quotes);
        })()
      : (() => res.json(quotes[~~(quotes.length * Math.random())]))()
  );
};
